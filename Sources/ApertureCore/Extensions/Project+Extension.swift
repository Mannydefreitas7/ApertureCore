...
extension Project {
    ...
    mutating func addTrack(type: Track.TrackType) {
        let count = tracks.filter { $0.type == type }.count + 1
        let name: String
        switch type {
            case .video: name = "Video \(count)"
            case .audio: name = "Audio \(count)"
            case .subtitle: name = "Subtile \(count)"
            case .effect: name = "Effect \(count)"
            case .overlay: name = "Overlay \(count)"
        }
        tracks.append(Track(type: type, name: name))
        modifiedAt = Date()
    }
